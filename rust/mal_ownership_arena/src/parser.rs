...
impl<'a> DeterministicParser<'a> {
    ...
    fn parse_factor<const L:usize,const A:usize>(&mut self,t:&mut FixedArena<LexicalToken<'a>,L>,a:&mut FixedArena<AstNode<'a>,A>)->Result<NodeID,ParseError>{let x=self.take(t)?.ok_or(ParseError::UnexpectedEof)?;match x.kind{TokenKind::Number(v)=>self.alloc(a,AstOpcode::LiteralNum,None,None,None,v),TokenKind::StringLiteral=>{let text=&x.slice[1..x.slice.len()-1];self.alloc(a,AstOpcode::LiteralString,Some(text),None,None,text.as_bytes().len() as u64)},TokenKind::LParen=>{let y=self.parse_comparison(t,a)?;self.expect(t,TokenKind::RParen)?;Ok(y)},TokenKind::RestrictedEvaluator=>Err(ParseError::ConstitutionalViolation),TokenKind::Symbol|TokenKind::KeywordNode|TokenKind::KeywordCount|TokenKind::KeywordCapacity|TokenKind::KeywordWeight|TokenKind::KeywordCalculate|TokenKind::KeywordString|TokenKind::KeywordAggregate|TokenKind::KeywordBound|TokenKind::KeywordAmount=>{if self.peek(t)?.is_some_and(|z|z.kind==TokenKind::LParen){self.take(t)?;let arg=self.parse_comparison(t,a)?;self.expect(t,TokenKind::RParen)?;let op=match x.kind{TokenKind::KeywordCount=>AstOpcode::QuranCount,TokenKind::KeywordCapacity=>AstOpcode::QuranCapacity,TokenKind::KeywordWeight=>AstOpcode::QuranWeight,TokenKind::KeywordCalculate|TokenKind::KeywordString=>AstOpcode::QuranWeight,TokenKind::KeywordAggregate=>AstOpcode::QuranAggregate,TokenKind::KeywordBound=>AstOpcode::QuranBound,TokenKind::KeywordAmount=>AstOpcode::QuranAmount,_=>AstOpcode::FunctionCall};self.alloc(a,op,Some(x.slice),Some(arg),None,0)}else{self.alloc(a,AstOpcode::BindSymbol,Some(x.slice),None,None,0)}},_=>Err(ParseError::SyntaxError)}}
