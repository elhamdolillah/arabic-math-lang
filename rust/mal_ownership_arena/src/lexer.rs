...
impl<'a> DeterministicLexer<'a> {
    ...
    pub fn tokenize_next<const N: usize>(
        ...
        let current = self.source.get(self.cursor..).ok_or(LexerError::NumericOverflow)?;
        let (slice, kind, consumed) = if current.starts_with("eval(") || current.starts_with("exec(") {
            let consumed = current.find(')').map_or_else(
                || current.char_indices().find_map(|(index, character)| character.is_whitespace().then_some(index)).unwrap_or(current.len()),
                |index| index + 1,
            );
            (&current[..consumed], TokenKind::RestrictedEvaluator, consumed)
        } else if current.starts_with("unsafe") {
            let consumed = current.char_indices().find_map(|(index, character)| character.is_whitespace().then_some(index)).unwrap_or(current.len());
            (&current[..consumed], TokenKind::RestrictedEvaluator, consumed)
        } else {
            ...
            if let Some(kind) = single {
                ...
                let consumed = current.char_indices().find_map(|(index, character)| {
                    (character.is_whitespace() || matches!(character, '=' | '+' | '-' | '*' | '/' | '(' | ')')).then_some(index)
                }).unwrap_or(current.len()).max(1);
                ...
                let kind = if slice.starts_with("بنية_") || slice == "NodeID" {
                    ...
                    let number = slice.parse::<u64>().map_err(|_| LexerError::NumericOverflow)?;
                    TokenKind::Number(number)
                } else if slice.chars().all(|character| character.is_alphanumeric() || character == '_' || character == 'ـ') {
                    TokenKind::Symbol
                } else {
                    TokenKind::Unknown
                };
        ...
        let id = arena.allocate(LexicalToken { kind, slice, node_id: next_id })?;
